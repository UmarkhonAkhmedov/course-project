...
function ItemsTable({ fetching }) {
  ...
  const fetchData = async () => {
    await axios
      .get("https://course-project-rgk2.vercel.app/items")
      .then((res) => {
        setData(res.data);
      });
  };
  ...
  const handleDeleteProperty = async () => {
    for (let i = 0; i < select.length; i++) {
      try {
        const url = `https://course-project-rgk2.vercel.app/items/delete/${select[i]}`;
  ...
  const handleEditProperty = async () => {
    try {
      const url = `https://course-project-rgk2.vercel.app/items/update/${editElement}`;
